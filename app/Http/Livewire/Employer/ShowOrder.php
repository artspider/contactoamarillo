...
class ShowOrder extends Component
{
    ...
    public function render()
    {
        ...
        logger('show order');
        $this->activitiesm = DB::table('activities')->where('order_id','=',$this->order->id)->get();
        return view('livewire.employer.show-order',[
            'order' => $this->order,
    ...
    {    
        ...
        $newActivity->sender = 1; //El employer envía mensaje
    ...
    public function acceptFinish()
    {
        $this->order->status = 6;
        $this->order->save();

        $newActivity = new Activity;
        $newActivity->message = "CloseitNow: ¡Has aceptado la entrega! La orden se ha marcado como completada y se ha cerrado";
        $newActivity->order_id = $this->order->id;
        $newActivity->sender = 1; //El employer envía mensaje
        $result = $newActivity->save();
        if($result){
        ...
        $this->emit('success', '¡Has aceptado la entrega satisfactoriamente!');
        $this->tab=1;
    }
}
